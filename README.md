...
## Modeling
We pre-processed the data by first filtering out English reviews only and from a particular game to limit the size of our data. We then represented our data using Bag of Words and TF-IDF to capture word frequencies/importance, respectively. After that, we created a Naive bayes classifier to model our data. We chose naive bayes to be the best approach since it assigns probabilities to words and allows us to find the best features people say are good about the game. However, we recognize that since most of the Payday reviews contains mostly positive reviews, the accuracy may not be representative of the data. 
